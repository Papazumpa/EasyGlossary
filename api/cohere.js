    ...
    if (req.method === 'POST') {
        ...
        try {
            const response = await client.chat({
                message: `Please correct spelling errors and remove any irrelevant text from this OCR output. I need the phrases paired next to each other, with the first languages phrase on the left and the corresponding second languages phrase on the right separated by a comma and every pair separaded by a period . (example output: hello,Bonjour.nice meeting you,Plaisir de vous rencontrer etc.). If a phrase is missing in one language, please leave the missing side blank. Here is the input text: ${ocrOutput}`,
